  ...
  <div class="wrap">
    ...
    <el-form :inline="true" :model="formInline" class="demo-form-inline" size="mini">
      ...
      <el-form-item label="品名">
        <el-input v-model="formInline.product_name"  @keydown.enter.native="onSubmit"/>
      </el-form-item>
    ...
    >
      <el-table-column prop label="订单号" align="center">
        <template slot-scope="scope">
          <span class="code" @click="getMaterialDetail(scope.row)">{{scope.row.order_no}}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column prop label="数量(收/总)" align="center">
        <template slot-scope="scope">
          {{ scope.row.receive_num?scope.row.receive_num:0}}/{{scope.row.buy_num}}
        </template>
      </el-table-column>
      <el-table-column prop label="采购时间" align="center" show-overflow-tooltip>
        <template slot-scope="scope">{{ scope.row.buy_time}}</template>
      </el-table-column>
      <!-- 添加加载插槽 -->
...
export default {
  ...
  methods: {
   ...
   async getData() {
      ...
      // 显示条数
      const { data : res } = await this.$http.get('/material/query_material_order',{
